...
class DQNAgent():
    ...
    def getLoss(self, states, actions, next_states, rewards, status,smoothFlag=0): ## added flag
        ...
        ###
        if(smoothFlag==0):
            q_train = self.training_net.forward(states) # already here
        else:
            q_train = self.smoothNet.forward(states)
        ###
        q_train = q_train.gather(1,actions.unsqueeze(1))
        q_target = self.target_net.forward(next_states).detach()
